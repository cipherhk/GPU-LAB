...
// CPU implementation
//////////////////////////////////////////////////////////////////////////////
void mandelbrotHost(std::vector<cl_uint>& h_output, size_t countX,
                    size_t countY, cl_uint niter, float xmin, float xmax,
...
int main(int argc, char** argv) {
  ...
  extern unsigned char OpenCLExercise2_Mandelbrot_cl[];
  extern unsigned int OpenCLExercise2_Mandelbrot_cl_len;
  cl::Program program(context,
                      std::string((const char*)OpenCLExercise2_Mandelbrot_cl,
                                  OpenCLExercise2_Mandelbrot_cl_len));
  ...
  int64_t
      maxError;  // maximum difference between CPU and GPU solution (to account for rounding errors)
  ...
  std::size_t countX =
      wgSizeX *
      128;  // Overall number of work items in X direction = Number of elements in X direction
  ...
  for (size_t i = 0; i < countX; i++) {
    for (size_t j = 0; j < countY; j++) {
      ...
      imageDataCpu[i + countX * (countY - j - 1)] =
          1 - 1.0f * h_outputCpu[i + j * countX] / (niter - 1);
      imageDataGpu[i + countX * (countY - j - 1)] =
          1 - 1.0f * h_outputGpu[i + j * countX] / (niter - 1);
    }
  }
  Core::writeImagePGM("output_mandelbrot_bw_cpu.pgm", imageDataCpu, countX,
                      countY);
  Core::writeImagePGM("output_mandelbrot_bw_gpu.pgm", imageDataGpu, countX,
                      countY);
  Core::writeImagePPM("output_mandelbrot_col_cpu.ppm", imageDataCpu, countX,
                      countY);
  Core::writeImagePPM("output_mandelbrot_col_gpu.ppm", imageDataGpu, countX,
                      countY);
  ...
  for (size_t i = 0; i < countX; i = i + 1) {    //loop in the x-direction
    for (size_t j = 0; j < countY; j = j + 1) {  //loop in the y-direction
      ...
      if (!(std::abs((int64_t)h_outputCpu[index] -
                     (int64_t)h_outputGpu[index]) <= maxError)) {
        if (errorCount < 15)
